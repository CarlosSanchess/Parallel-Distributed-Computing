...
public class TimeServer {
    ...
    private void initializeNextClientId() {
        try {
            ...
            nextClientId = credentials.values().stream()
                .mapToInt(creds -> Integer.parseInt(creds[0]))
                .max()
                .orElse(-1) + 1;
        } catch (IOException e) {
            System.out.println("[INFO] Starting with fresh client IDs");
            nextClientId = 0;
        }
    }
        ...
        try {
            ...
            try {
                ...
                if (c != null) {
                    ...
                    } else if (c.getState() == ClientState.IN_ROOM) {
