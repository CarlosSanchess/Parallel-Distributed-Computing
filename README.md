...
##  Assign 1
For assign 1(Performance Comparison of Single-Core and Multi-Core Systems) the documentation is available under the assign1/doc folder.
...
The system provides robust security through SSL/TLS encryption using the `javax.net.ssl` package. This implementation features:

- **End-to-End Encryption**: All communication is encrypted using TLS 1.3 protocols
- **Certificate-Based Authentication**: Uses JKS keystores for server authentication
- **Strong Cipher Suites**: Automatically filters out weak encryption algorithms (NULL, anon, DES, RC4)
- **Environment-Based Configuration**: Keystore path and password can be configured via environment variables

### SSL Configuration

The server requires a JKS keystore file for SSL operation:
- Default keystore path: `server.keystore` (configurable via `TIMESERVER_KEYSTORE_PATH` environment variable)
- Keystore password: Set via `TIMESERVER_KEYSTORE_PASSWORD` environment variable
- The server uses TLS 1.3 protocol with strong cipher suites only
- Client authentication is optional (server does not require client certificates)

### Security Features
- **Automatic Cipher Suite Filtering**: Excludes weak encryption algorithms
- **Configurable Certificate Management**: Supports environment-based keystore configuration
- **SSL Handshake Verification**: Ensures secure connection establishment before processing requests
...
Before running the server, you need to generate SSL certificates and configure environment variables:

#### Generate SSL Certificates
```bash
# Make the script executable
chmod +x secure-keystore-gen.sh

# Run the keystore generation script
./secure-keystore-gen.sh
```

The script will prompt you for:
- Keystore password (minimum 6 characters)
- Key password (minimum 6 characters) 
- Truststore password (minimum 6 characters)

#### Set Environment Variables
```bash
# For the server
source timeserver.env

# For the client (in a separate terminal)
source client.env
```
...
# Set SSL environment variables and run
source timeserver.env
...
```bash
# Compile the client
javac --enable-preview -source 21 -Xlint:preview TimeClient.java

# Set SSL environment variables and run
source client.env
java --enable-preview TimeClient localhost 8888
```
